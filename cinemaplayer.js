...
router.get('/:tab', function(req, res) {
  ...
  var ip =
    ...
    decodeURIComponent('' + req.query.ip).replace(/[^0-9a-fA-F:.%]/g, '')
      ? decodeURIComponent('' + req.query.ip).replace(/[^0-9a-fA-F:.%]/g, '')
      : '';
  var hash =
...
function getIp(req) {
  ...
  ips.forEach(function(one_ip) {
    ...
    if (
      one_ip !== '127.0.0.1' &&
      (
        /^([01]?[0-9]?[0-9]|2[0-4][0-9]|25[0-5])\.([01]?[0-9]?[0-9]|2[0-4][0-9]|25[0-5])\.([01]?[0-9]?[0-9]|2[0-4][0-9]|25[0-5])\.([01]?[0-9]?[0-9]|2[0-4][0-9]|25[0-5])$/.test(one_ip) ||
